...
int main(int argc, char* argv[])
{
  try
  {
    TCLAP::CmdLine cmd("Subscriber_2", ' ', "1.0");
    ...
    std::cout << "[Subscriber 2] Waiting for messages..." << std::endl;
    ...
    {
      std::cout << "[Subscriber 2] [✓] Received messages from both publishers.\n" << std::endl;
      return 0;
    }
    else
    {
      std::cerr << "[Subscriber 2] [✗] Missing messages from one or both publishers.\n" << std::endl;
      return 1;
    }
  }
  catch (const TCLAP::ArgException& e)
  {
    std::cerr << "[Subscriber 2] TCLAP error: " << e.error() << " (arg: " << e.argId() << ")" << std::endl;
    return 1;
  }
}
