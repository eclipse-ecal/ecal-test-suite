...
int main(int argc, char* argv[])
{
  try
  {
    TCLAP::CmdLine cmd("Publisher_1", ' ', "1.0");
    ...
    std::cout << "[Publisher 1] Started with mode=" << mode
              << ", topic=" << topic
              << ", node=" << node_name
              << ", count=" << count
    ...
    std::cout << "\n[Publisher 1] Finished sending messages." << std::endl;
  ...
  {
    std::cerr << "\n[Publisher 1] TCLAP error: " << e.error() << " (arg: " << e.argId() << ")" << std::endl;
    return 1;
  }
}
